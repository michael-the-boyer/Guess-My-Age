...
function addressGuess () {
  ...
  let guess = prompt('Guess my age here!', '')
  guess = parseInt(guess)
  while (guess !== randomAge) {
    if (guess < randomAge) {
      guess = prompt('Hmm, your guess is too low, try guessing higher.', '')
      guess = parseInt(guess)
    } else if (guess > randomAge) {
      guess = prompt('Woah, your guess is too high! Lower! Lower!', '')
      guess = parseInt(guess)
    } else {
      guess = prompt('Something went wrong, make sure you are plugging in a numerical value.', '')
    }
  }
